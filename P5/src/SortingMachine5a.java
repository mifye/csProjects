...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        // if not at last child of heap
        if (top != last) {
            if (order.compare(array[top], array[top * 2 + 1]) <= 0
                    && top * 2 + 1 <= last) {
                // if left subtree exists and
                // if top is <= left subtree root, -> swap top with left subtree root
                // maintaining; left <= middle <= right
                exchangeEntries(array, top, top * 2 + 1);
                siftDown(array, top * 2 + 1, last, order);

            } else if (order.compare(array[top], array[top * 2 + 2]) >= 0
                    && top * 2 + 2 <= last) {
                // if right subtree exists and
                // else if top is >= right subtree root, -> swap top with right root
                // maintaining; left <= top <= right
                exchangeEntries(array, top, top * 2 + 1);
                siftDown(array, top * 2 + 2, last, order);
            }
            // if top is neither <= left root or >= right root, it is already ordered
        }
    ...
    private static <T> void heapify(T[] array, int top, Comparator<T> order) {
        ...
        // *** you must use the recursive algorithm discussed in class ***
        if (top * 2 + 2 <= array.length) { // right subtree implies left
            heapify(array, top * 2 + 1, order);
            heapify(array, top * 2 + 2, order);
            siftDown(array, top, array.length, order);
        } else if (top * 2 + 1 <= array.length) { // only left subtree
            heapify(array, top * 2 + 1, order);
            siftDown(array, top, array.length, order);
        }
    ...
    public final void changeToExtractionMode() {
        ...
        buildHeap(this.entries, this.machineOrder);
